...
def update_best_loss(model_type ,spatial_mode ,fips,best_loss,X_train_train_to_use,X_train_val_to_use,y_train_train,\
    ...
    if model_type == 'mixed_model':
          ...
          if (spatial_mode == 'county') or (spatial_mode == 'state') :
            # Construct the outputs for the testing dataset of the 'MM' methods
            y_predictions_test.extend([y_prediction[fips]['GBM'][(h, c)], y_prediction[fips]['GLM'][(h, c)],
                                        y_prediction[fips]['KNN'][(h, c)], y_prediction[fips]['NN'][(h, c)]])
          ...
          if (spatial_mode == 'county') or (spatial_mode == 'state') :
            ...
            y_predictions_train.extend([y_prediction_train[fips]['GBM'][(h, c)], y_prediction_train[fips]['GLM'][(h, c)],
                                        y_prediction_train[fips]['KNN'][(h, c)], y_prediction_train[fips]['NN'][(h, c)]])
          elif spatial_mode == 'country':
    ...
    if model_type == 'none_mixed_model':
          ...
          if spatial_mode == 'country':
            ...
            loom.add_function(NN_grid_search, [X_train_train_to_use['NN'][covariates],
                                                    ...
                                                    y_train_val])
          if (spatial_mode == 'county') or (spatial_mode == 'state') :
            loom.add_function(GBM_grid_search, [X_train_train_to_use[fips][h]['GBM'][covariates],
                                                    y_train_train , X_train_val_to_use[fips][h]['GBM'][covariates],
                                                    y_train_val])
            loom.add_function(NN_grid_search, [X_train_train_to_use[fips][h]['NN'][covariates],
                                                    y_train_train , X_train_val_to_use[fips][h]['NN'][covariates],
                                                    y_train_val])
          best_loss_output=loom.execute()
...
def get_best_loss_mode(counties_best_loss_list):
  ...
  for method in methods_with_loss:
    ...
    # some times its not possible to find mode
...
def generate_data(h, numberOfCovariates, covariates_names, numberOfSelectedCounties):
    ...
    best_covariates += ['county_fips','state_fips','date of day t'] # we add this two columns to use when we want break data to county_data
...
def test_process(h, r, target_name,spatial_mode, target_mode,best_h,best_c,historical_X_train,\
    ...
    for state_fips in all_states:
        ...
        GBM, GLM, KNN, NN = run_algorithms(historical_X_train, historical_X_test, historical_y_train_date, historical_y_test_date, best_loss, 0, spatial_mode, state_fips)

        y_prediction[state_fips]['GBM'], y_prediction_train[state_fips]['GBM'] = GBM
        y_prediction[state_fips]['GLM'], y_prediction_train[state_fips]['GLM'] = GLM
    ...
    y_prediction_train = {state_fips: {'GBM': {}, 'GLM': {}, 'KNN': {}, 'NN': {}, 'MM_GLM': {}, 'MM_NN': {}}
                    for state_fips in all_states}
    # #table_data = []
    X_train_MM_dict = {state_fips: {'GBM': {}, 'GLM': {}, 'KNN': {}, 'NN': {}, 'MM_GLM': {}, 'MM_NN': {}}
                    for state_fips in all_states}
    X_test_MM_dict = {state_fips: {'GBM': {}, 'GLM': {}, 'KNN': {}, 'NN': {}, 'MM_GLM': {}, 'MM_NN': {}}
                    for state_fips in all_states}
    y_train_MM_dict = {state_fips: {'GBM': {}, 'GLM': {}, 'KNN': {}, 'NN': {}, 'MM_GLM': {}, 'MM_NN': {}}
                    for state_fips in all_states}
    y_test_MM_dict = {state_fips: {'GBM': {}, 'GLM': {}, 'KNN': {}, 'NN': {}, 'MM_GLM': {}, 'MM_NN': {}}
                    for state_fips in all_states}
    y_train, y_test = {}, {}
    y_test_date = {}
    ...
    for mixed_method in mixed_methods:
        ...
        # we run mixed model for each state in this loop
    
        # for mixed_method in mixed_methods:

        for state_fips in all_states:

            GBM, GLM, KNN, NN = run_algorithms(X_train_dict, X_test_dict, y_train_dict, y_test_dict, best_loss, 0, spatial_mode, state_fips)
            y_prediction[state_fips]['GBM'], y_prediction_train[state_fips]['GBM'] = GBM
            y_prediction[state_fips]['GLM'], y_prediction_train[state_fips]['GLM'] = GLM
            y_prediction[state_fips]['KNN'], y_prediction_train[state_fips]['KNN'] = KNN
            y_prediction[state_fips]['NN'], y_prediction_train[state_fips]['NN'] = NN
            y_predictions_test, y_predictions_train = [], []
            # Construct the outputs for the testing dataset of the 'MM' methods
            y_predictions_test.extend([y_prediction[state_fips]['GBM'], y_prediction[state_fips]['GLM'], y_prediction[state_fips]['KNN'], y_prediction[state_fips]['NN']])
            y_prediction_test_np = np.array(y_predictions_test).reshape(len(y_predictions_test), -1)
            X_test_mixedModel = pd.DataFrame(y_prediction_test_np.transpose())
            # Construct the outputs for the training dataset of the 'MM' methods
            y_predictions_train.extend([y_prediction_train[state_fips]['GBM'], y_prediction_train[state_fips]['GLM'], y_prediction_train[state_fips]['KNN'], y_prediction_train[state_fips]['NN']])
            y_prediction_train_np = np.array(y_predictions_train).reshape(len(y_predictions_train), -1)
            X_train_mixedModel = pd.DataFrame(y_prediction_train_np.transpose())
            X_train_MM_dict[state_fips][mixed_method] = X_train_mixedModel
            X_test_MM_dict[state_fips][mixed_method] = X_test_mixedModel
            y_train_MM_dict[state_fips][mixed_method] = y_train[mixed_method][y_train[mixed_method]['state_fips']==state_fips]
            y_test_MM_dict[state_fips][mixed_method] = y_test[mixed_method][y_test[mixed_method]['state_fips']==state_fips]


            y_test_MM_dict[state_fips][mixed_method] = np.array(y_test_MM_dict[state_fips][mixed_method]['Target']).reshape(-1)
            y_train_MM_dict[state_fips][mixed_method] = np.array(y_train_MM_dict[state_fips][mixed_method]['Target']).reshape(-1)

    for state_fips in all_states:

        # mixed model with linear regression and neural network
        MM_GLM, MM_NN = run_mixed_models(X_train_MM_dict[state_fips], X_test_MM_dict[state_fips], y_train_MM_dict[state_fips], y_test_MM_dict[state_fips] ,best_loss)
        y_prediction[state_fips]['MM_GLM'], y_prediction_train[state_fips]['MM_GLM'] = MM_GLM
        y_prediction[state_fips]['MM_NN'], y_prediction_train[state_fips]['MM_NN'] = MM_NN
...
def flatten(data=None, h=None, c=None, method=None, covariates_list=None, state=1):
    if state == 1:
        result = []
        for state_fips in data:
            result += list(data[state_fips][method][(h, c)])
    elif state == 2:
        result = []
        for state_fips in data:
            result += list(data[state_fips][(h, c)])
    elif state == 3:
        result = pd.DataFrame(columns=covariates_list)
        for state_fips in data:
            result = pd.concat([result, data[state_fips][h][method][covariates_list]], ignore_index=True)
    elif state == 4:
        for state_fips in data:
            result = pd.DataFrame(columns=data[state_fips].columns.values)
            break
        for state_fips in data:
            result = pd.concat([result, data[state_fips]], ignore_index=True)
    elif state == 5:
        result = []
        for state_fips in data:
            result += list(data[state_fips])
        result = np.array(result)
    elif state == 6:
        result = []
        for state_fips in data:
            result += list(data[state_fips][method])
    return result
...
def main(maxHistory, maxC):
    ...
    for h in history:
        ...
        y_test = {state_fips: None for state_fips in all_states}
        for state_fips in all_states:
            print("state_fips =", state_fips)
            data = all_data[all_data['state_fips']==state_fips]
            parallel_outputs = {}
            X_train_train, X_train_val, X_test, y_train_train_date, y_train_val_date, y_test_date[state_fips] = preprocess(data, spatial_mode, 1)
            indx_c = 0
            for c in covariates_names:
                indx_c += 1
                y_val[state_fips][(h, indx_c)] = np.array(y_train_val_date['Target']).reshape(-1)
                if indx_c == maxC:
                    break
            for method in methods:
                X_train_train_to_use[state_fips][h][method] = X_train_train.copy()
                X_train_val_to_use[state_fips][h][method]= X_train_val.copy()
                X_test_to_use[state_fips][h][method] = X_test.copy()
                if method in models_to_log:
                    negative_features=['temperature']
                    for covar in covariates_names:
                        if (' t' in covar) and (covar.split(' ')[0] not in negative_features):
                            X_train_train_to_use[state_fips][h][method][covar] = np.log((X_train_train_to_use[state_fips][h][method][covar] + 1).astype(float))
                            X_train_val_to_use[state_fips][h][method][covar] = np.log((X_train_val_to_use[state_fips][h][method][covar] + 1).astype(float))
                            X_test_to_use[state_fips][h][method][covar] = np.log((X_test_to_use[state_fips][h][method][covar] + 1).astype(float))
                    fix_log_list = ['total_population', 'population_density', 'area', 'median_household_income',
                                    'houses_density', 'airport_distance','deaths_per_100000']
                    for covar in fix_log_list:
                        if covar in covariates_names:
                            ...
                            X_train_val_to_use[state_fips][h][method][covar] = np.log((X_train_val_to_use[state_fips][h][method][covar] + 1).astype(float))
                            X_test_to_use[state_fips][h][method][covar] = np.log((X_test_to_use[state_fips][h][method][covar] + 1).astype(float))
            y_train_date[state_fips] = (pd.DataFrame(y_train_train_date).append(pd.DataFrame(y_train_val_date))).reset_index(drop=True)
            y_train_train = np.array(y_train_train_date['Target']).reshape(-1)
            y_train_val = np.array(y_train_val_date['Target']).reshape(-1)
            y_test[state_fips] = np.array(y_test_date[state_fips]['Target']).reshape(-1)
            y_train[state_fips] = np.array((pd.DataFrame(y_train_train).append(pd.DataFrame(y_train_val))).reset_index(drop=True)).reshape(-1)
            # find best loss
            if (h==1):
              best_loss = update_best_loss('none_mixed_model', spatial_mode ,state_fips,best_loss,X_train_train_to_use,X_train_val_to_use,\
              ...
              for method in ['GBM', 'NN']:
                states_best_loss_list[method].append(best_loss[method])
            covariates_list = []
            ...
            for c in covariates_names:
                ...
                for method in none_mixed_methods:
                    X_train_train_temp = X_train_train_to_use[state_fips][h][method][covariates_list]
                    X_train_val_temp = X_train_val_to_use[state_fips][h][method][covariates_list]
            ...
            for c in range(1, numberOfCovariates + 1):
                for method in none_mixed_methods:
                    y_prediction[state_fips][method][(h, c)], y_prediction_train[state_fips][method][(h, c)] = parallel_outputs['non_mixed'][ind]['output']
                    ind += 1
                if c == maxC:
                    break
            ...
            if h == 1 :
              best_loss = update_best_loss('mixed_model', spatial_mode, state_fips,best_loss,None,None,y_train_train,\
                        ...
                        numberOfCovariates,maxC)
              # update list of county losses (mode of this list will be used as best loss)
              states_best_loss_list['MM_NN'].append(best_loss['MM_NN'])
            ...
            for c in range(1, numberOfCovariates + 1):
                ...
                for mixed_method in mixed_methods:
                    y_predictions_test, y_predictions_train = [], []
                    y_predictions_test.extend([y_prediction[state_fips]['GBM'][(h, c)], y_prediction[state_fips]['GLM'][(h, c)],
                                                y_prediction[state_fips]['KNN'][(h, c)], y_prediction[state_fips]['NN'][(h, c)]])
                    y_prediction_test_np = np.array(y_predictions_test).reshape(len(y_predictions_test), -1)
                    X_test_mixedModel = pd.DataFrame(y_prediction_test_np.transpose())
                    y_predictions_train.extend([y_prediction_train[state_fips]['GBM'][(h, c)], y_prediction_train[state_fips]['GLM'][(h, c)],
                                                y_prediction_train[state_fips]['KNN'][(h, c)], y_prediction_train[state_fips]['NN'][(h, c)]])
                    y_prediction_train_np = np.array(y_predictions_train).reshape(len(y_predictions_train), -1)
                    X_train_mixedModel = pd.DataFrame(y_prediction_train_np.transpose())
            ...
            for c in range(1, numberOfCovariates + 1):
                for mixed_method in mixed_methods:
                    y_prediction[state_fips][mixed_method][(h, c)], y_prediction_train[state_fips][mixed_method][(h, c)] = parallel_outputs['mixed'][ind]['output']
                    y_prediction[state_fips][mixed_method][(h, c)] = np.array(y_prediction[state_fips][mixed_method][(h, c)]).ravel()
                    y_prediction_train[state_fips][mixed_method][(h, c)] = np.array(y_prediction_train[state_fips][mixed_method][(h, c)]).ravel()
                    ind += 1
                if c == maxC:
                    break
            filename = env_address + 'validation.out'
        ...
        indx_c = 0
        covariates_list=['county_fips','date of day t', 'state_fips']
        for c in covariates_names:
            print(indx_c)
            indx_c += 1
            for covariate in data.columns:
            ...
            if indx_c == maxC:
                break
        if h == 1:
          best_loss = get_best_loss_mode(states_best_loss_list)
        ...
        if (number_of_improved_methods == 0) or (h == maxHistory//2) or (h == 3) :
    ...
    print(len(y_prediction[1]['GBM'][(1, 1)]))
    print("y_val", y_val)
