...
def mase_denominator(r, h, target_name, target_mode ,numberOfSelectedCounties, spatial_mode):
    ...
    data = makeHistoricalData(h, r, target_name, 'mrmr', 'country', target_mode, './')
    if numberOfSelectedCounties == -1 :
      numberOfSelectedCounties = len(data['county_fips'].unique())
...
def generate_data(h, numberOfCovariates, covariates_names, numberOfSelectedCounties):
    ...
    if spatial_mode=='state':
      best_covariates += ['county_fips','date of day t','state_fips']# we add this two columns to use when we want break data to county_data
    else:
...
def get_errors(h, c, method, y_prediction, y_prediction_train, y_test_date, MASE_denominator, numberOfSelectedCounties, target_name, mode):
    ...
    if  target_mode=='cumulative':
        ...
        if mode == 'val':
            ...
            # we may have not enough days (2*r(2*seasonality)) to calculate MASE error in all counties for validation
            # in that case we set MASE error to -1 for validation to run code without error and it dosent corrupt
            # process of learning because we use MAPE for selecting best h and c
            
            count=X_train_train_new_case.groupby(['county_fips']).count() # check number of days we have for all counties
            if len(count[count['date of day t']<2*r])>0: 
                MASE=-1
            else:
                X_train_train_new_case=X_train_train_new_case.drop(['date of day t', 'county_fips'], axis=1)
                ...
                print('cumul_train_train_predict',cumul_train_train_predict.shape)
                for i in range(r):
                    ...
                    X_train_train_new_case_temp=X_train_train_new_case_temp.sort_values(by=['county_fips','date of day t'])
    
                    print(i,'i')
                    #print('X_train_train_new_case_temp',X_train_train_new_case_temp)
                    print(X_train_train_new_case_temp['county_fips'].value_counts())
        ...
        if mode == 'test':
            ...
            for i in range(r):
                ...
                if i==0:
                  X_train_new_case_temp = X_train_new_case_temp.tail(numberOfSelectedCounties*r)
                else:
                  X_train_new_case_temp = X_train_new_case_temp.iloc[:-(numberOfSelectedCounties*(i)), :].tail(numberOfSelectedCounties*r)
                X_train_new_case_temp = X_train_new_case_temp.sort_values(by=['county_fips','date of day t'])
                print('X_train_train_new_case_temp',X_train_new_case_temp)
...
def make_mixed_model_data(X_train_dict, X_test_dict, y_train_dict, y_test_dict, y_train, y_test,
                                  ...
                                  best_loss, spatial_mode, mixed_method, state_fips):
            ...
            fips_y_train_MM_dict = y_train[mixed_method][y_train[mixed_method]['state_fips']==state_fips]
            fips_y_test_MM_dict = y_test[mixed_method][y_test[mixed_method]['state_fips']==state_fips]
...
def test_process(h, r, target_name,spatial_mode, target_mode,best_h,best_c,historical_X_train,\
    ...
    all_state_data = pd.read_csv('./fixed-data.csv')
    all_states = all_state_data['state_fips'].unique()
    print(all_states)
    ...
    if __name__ == '__main__':
          ...
          parallel_output = run_algorithms_Pool.map(partial(run_algorithms, historical_X_train, historical_X_test, historical_y_train_date, 
                                              historical_y_test_date, best_loss, 0, spatial_mode),  list(all_states))
    ...
    for method in none_mixed_methods:
        ...
        best_c[method]['MAPE'], method, flatten(data=y_prediction, h=h, c=None, method=method, state=6), 
        flatten(data=y_prediction_train, h=h, c=None, method=method, state=6), historical_y_test_date[method].drop(['state_fips'],axis=1),
    ...
    for mixed_method in mixed_methods:
        ...
        mixed_method_all_states = X_train['state_fips'].unique()
        print(mixed_method, mixed_method_all_states)                                                      
        ...
        if __name__ == '__main__':
            ...
            parallel_output = make_data_Pool.map(partial(make_mixed_model_data, X_train_dict, X_test_dict, y_train_dict, y_test_dict, y_train, y_test,
                                  y_prediction_train, y_prediction,
                                  best_loss, spatial_mode, mixed_method),  list(mixed_method_all_states))

        for index , state_fips in enumerate(mixed_method_all_states):


            X_train_MM_dict[state_fips][mixed_method]=parallel_output[index][0]
            print(mixed_method,state_fips,'X_train_MM_dict[state_fips][mixed_method].shape',X_train_MM_dict[state_fips][mixed_method].shape)
            X_test_MM_dict[state_fips][mixed_method]=parallel_output[index][1]
            print(mixed_method,state_fips,'X_test_MM_dict[state_fips][mixed_method].shape',X_test_MM_dict[state_fips][mixed_method].shape)
            y_train_MM_dict[state_fips][mixed_method]=parallel_output[index][2]
    ...
    if __name__ == '__main__':
        ...
        parallel_output = run_mixed_models_Pool.map(partial(run_mixed_models, X_train_MM_dict, X_test_MM_dict,
                                            y_train_MM_dict, y_test_MM_dict ,best_loss, spatial_mode), list(all_states))
    ...
    #####################
    for mixed_method in mixed_methods:
        print(mixed_method,'y_prediction',len(flatten(data=y_prediction, h=h, c=None, method=mixed_method, state=6)))
        print(mixed_method,'y_prediction_train',len(flatten(data=y_prediction_train, h=h, c=None, method=mixed_method, state=6)))
        ...
        flatten(data=y_prediction_train, h=h, c=None, method=mixed_method, state=6), y_test_date[mixed_method].drop(['state_fips'],axis=1),
...
def validation_process(all_data,spatial_mode,covariates_names,best_loss,target_name,h,maxC,numberOfSelectedCountiesname,maxHistory,history,state_fips):
        ...
        for c in covariates_names:
            indx_c += 1
            fips_y_val[(h, indx_c)] = np.array(y_train_val_date['Target']).reshape(-1)
            if indx_c == maxC:
                break
        for method in methods:
        ...
        for c in covariates_names:
            ...
            print('\n\n none_mixed_methods for h = ',h,' c = ',indx_c,' fips = ',state_fips,' done \n\n')
            if indx_c == maxC:
                break
        ...
        for c in range(1, numberOfCovariates + 1):
            ...
            print('\n\n mixed_methods for h = ',h,' c = ',indx_c,' fips = ',state_fips,' done \n\n')
            if indx_c == maxC:
                break
        ...
        return fips_X_train_train_to_use, fips_X_train_val_to_use ,fips_X_test_to_use ,\
                ...
                fips_y_train ,fips_y_test,fips_y_val , best_loss
...
def main(maxHistory, maxC):
    ...
    for h in history:
        ...
        for index , state_fips in enumerate(all_states):
            ...
            X_train_val_to_use[state_fips][h]=parallel_output[index][1]
            X_test_to_use[state_fips][h]=parallel_output[index][2]
            y_prediction_train[state_fips]=parallel_output[index][3]
            y_prediction[state_fips]=parallel_output[index][4]
            y_test_date[state_fips]=parallel_output[index][5]
            y_train_date[state_fips]=parallel_output[index][6]
            y_train[state_fips]=parallel_output[index][7]
            y_test[state_fips]=parallel_output[index][8]
            y_val[state_fips]=parallel_output[index][9]
