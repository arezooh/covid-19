...
def test_process(h, r, target_name,spatial_mode, target_mode,best_h,best_c,historical_X_train,\
    ...
    for method in none_mixed_methods:
        ...
        flatten(data=y_prediction_train, h=h, c=None, method=method, state=6), historical_y_test_date[method].drop(['state_fips'],axis=1),
          historical_y_train_date[method].drop(['state_fips'],axis=1), val_test_MASE_denominator[best_h[method]['MAPE']], numberOfSelectedCounties, target_name, mode='test')
    ...
    for mixed_method in mixed_methods:
        ...
        flatten(data=y_prediction_train, h=h, c=None, method=mixed_method, state=6), y_test_date[mixed_method].drop(['state_fips'],axis=1),\
                 y_train[mixed_method].drop(['state_fips'],axis=1) ,val_test_MASE_denominator[best_h[mixed_method]['MAPE']] ,\
...
def main(maxHistory, maxC):
    ...
    for h in history:
        ...
        y_train_train_date = {county_fips: None for county_fips in all_states}
        ...
        for index , state_fips in enumerate(all_states):
            ...
            y_train_train_date[state_fips]=parallel_output[index][9]
            y_val[state_fips]=parallel_output[index][10]
            
            if h == 1:
              # update list of county losses (mode of this list will be used as best loss)
              for method in ['GBM', 'NN', 'MM_NN']:
                states_best_loss_list[method].append(parallel_output[index][11][method])
