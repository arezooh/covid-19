...
def makeHistoricalData(h, r, test_size, target, feature_selection, spatial_mode, target_mode,
    ...
    if h == 0:
        h = 1
    data_address = '../data/'  # ./
    future_mode = True  ####################################################### future mode is True
    ...
    timeDeapandantData['weekend'] = timeDeapandantData['date'].apply(
        lambda x: datetime.datetime.strptime(x, '%m/%d/%y'))
    timeDeapandantData['weekend'] = timeDeapandantData['weekend'].apply(lambda x: x.weekday())
    timeDeapandantData['weekend'] = timeDeapandantData['weekend'].replace([0, 1, 2, 3, 4, 5, 6],
                                                                          [-2, 0, 0, 0, 0, -1, -3])
    ...
    if mobility_flag:
        mobility = pd.read_csv(address+'United States.csv')
