...
def makeHistoricalData(h, r, test_size, target, feature_selection, spatial_mode, target_mode,
    ...
    pd.DataFrame(np.array(ix),columns=['name']).to_csv("./"+str(r)+"/covariates_rank.csv",index=False)
...
def main():
    ...
    future_features = []
    test_size = 21
    result = makeHistoricalData(h, r, test_size, target, 'mrmr', spatial_mode, target_mode, address,
