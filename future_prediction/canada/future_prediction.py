...
def splitData(numberOfCounties, main_data, target, spatial_mode, mode, r, temporal_mode):
...
########################################################### preprocess
def preprocess(main_data, spatial_mode, validationFlag, r, temporal_mode):
...
def create_tables(y_prediction, y_test_date, y_train_date, r, country_errors, temporal_mode, daily_output,
                  weekly_output, target_mode, run_code, current_date, scenario_flag, futures, covered_r):
    ...
    if temporal_mode == "weekly" :
        observed = date_nominalize(observed,temporal_mode,'Date')
        unobserved = date_nominalize(unobserved,temporal_mode,'Date')
        if first_run == 1 and r == weekly_r[0]:
            weekly_output = pd.concat([observed,weekly_output], ignore_index=True)
            
        weekly_output = pd.concat([weekly_output,unobserved.tail(covered_r[r])], ignore_index=True)
...
def find_best_configuration(address):
    ...
    for r in range(1,10+1):
        ...
        covariates = pd.read_csv("./"+str(r)+"/covariates_rank.csv").loc[:,'name'].tolist()

        val_address = address+str(r)+'/results/counties=1535/validation/tables/tabel_of_best_validation_results.csv'
        test_address = address+str(r)+'/results/counties=1535/test/tables/table_of_best_test_results.csv'
...
def create_site_csv(data, temporal_mode):
    site_csv = data
    if temporal_mode == 'weekly':
    ...
    site_csv = site_csv[['date','predicted_value','real_value','model_number','death_or_confirmed','location_level','country_number','state_number','county_number']]
...
def main():
    ...
    for r in range(1,10+1):
        print(200 * '*')
        print('r = ',r)
##        subprocess.call("python ./prediction.py "+str(r), shell=True)

    updated_best_r,updated_best_methods,updated_best_covariates,updated_best_h,updated_best_error,covered_r = find_best_configuration(address)
    ...
    #     selected_covariates={
    #        'd14':all_covariates[:best_c['d14']], 'm14':all_covariates[:best_c['m14']], 'b14':all_covariates[:best_c['b14']],         
    #        'w28':future_all_covariate[:best_c['w28']], 's28':future_all_covariate[:best_c['s28']]}
    
        print('current_date = '+ datetime.datetime.strftime(current_date,'%d/%m/%y'))

        ####################### reading data and get errors
        
        data,regular_data = read_data(h, r, test_size, target_name, target_mode, future_features, pivot, current_date, run_code)
...
#         regular_data.to_csv(str(h)+'-'+str(r)+'regular_data.csv',index=False)
        ...
        X_train, _, y_train_date, _ = preprocess(data,spatial_mode,0, r, temporal_mode)

        # X_train, _, _, y_train_date, _, _ = preprocess(data,spatial_mode,1, r, temporal_mode)

        X_X_train, X_test , y_y_train_date, y_test_date = preprocess(data,spatial_mode,0, r, temporal_mode)
        ...
        scenarios_list = ['SD in 4 weeks ago', 'minimum grade', 'medium grade', 'maximum grade']
        ...
        if run_code == 'd14':
            daily_output, _ = create_tables(y_prediction, y_test_date, y_train_date, r, country_errors, temporal_mode,
                                            ...
                                            scenario_flag = 0, futures = selected_futures, covered_r = covered_r)
...
##    daily_output_csv = daily_output_csv.drop_duplicates(subset = ['the day of the target variable'],keep='last')
    ...
    weekly_output_csv.to_excel("Weekly-Deaths-Prediction.xlsx", index=False)
    
#     daily_site_csv = create_site_csv(daily_output_csv, 'daily')
#     daily_site_csv.to_csv("site_file_daily_Iran.csv", index=False)
    
#     weekly_site_csv = create_site_csv(weekly_output_csv, 'weekly')
#     weekly_site_csv.to_csv("site_file_weekly_Iran.csv", index=False)
